...
This document assumes that you are familiar with the link:/anypoint-studio/v/6/[Visual Editor in Anypoint Studio] that you can use to create applications in Studio. Further, this document references link:/mule-user-guide/v/3.8/global-elements[Global Elements] and the category of message processors known as link:/mule-user-guide/v/3.8/scopes[scopes].  Review link:/anypoint-studio/v/6/[Anypoint Studio Essentials] to learn more about developing with Mule's graphical user interface. Also review link:/getting-started/anypoint-exchange#accessing-the-exchange-s-public-content[Anypoint Exchange] for how to locate and install a connector using Exchange.
...
+
. Drag an element inside the poll scope, for example a *Database* connector. Mule uses this message processor to regularly poll the Database for new data to process. +
+
...
http://www.mulesoft.org/schema/mule/ee/tracking http://www.mulesoft.org/schema/mule/ee/tracking/current/mule-tracking-ee.xsd">
    ...
    <flow name="pollerFlow" processingStrategy="synchronous">
        <poll doc:name="Poll">
            ...
            <watermark variable="lastUpdatedVal" default-expression="0" selector="MAX" selector-expression="#[payload.LastUpdated]"/>
            <db:select config-ref="MySQL_Configuration" doc:name="Database">
                <db:parameterized-query><![CDATA[SELECT * FROM Employees where LastUpdated > #[flowVars.lastUpdatedVal];]]></db:parameterized-query>
            </db:select>
        </poll>
...
. Follow the steps above to create a flow that polls Database for data every 1000 milliseconds, then logs the message payload.
+
...
+
. Click the Database Connector and include a Where clause to incorporate *LastUpdated* to: +
+
`#[flowVars['lastUpdatedVal']]`
+
This value for the attribute instructs the connector to return only those rows that have an value in column LastUpdated greater than the value of the `lastUpdatedVal` variable. `lastUpdatedVal` is a flow variable that Mule creates, then updates every time the poll runs.
+
...
• A flow variable that the Connector references in query that it executes.
...
[source]
----
variable="lastUpdatedVal"
----

|*Default Expression* |The value that `lastUpdatedVal` uses the first time Mule executes the poll, or whenever the watermark can't be found.
...
|*Selector* |Pick the MAX value returned by the Selector Expression to update the `lastUpdatedVal` variable each time the flow execution completes. In this case, it takes the maximum of the LastUpdated value in the generated output (that is, the most recent one).
...
|*Selector expression* |Return the LastUpdated date of each record in the generated output, this value is passed on to the Selector.

*Value*: `#[payload.LastUpdated]` +
*XML*: +

[source]
----
selector-expression="#[payload.LastUpdated]"
----
...
----
+
.. `variable="lastUpdatedVal"` - String - Mule creates two variables: +
+
** A persistent object store variable with the provided name.
** A flow variable that the Database Connector references in its parameterized query.
+
.. `default-expression= "0"` - Integer - The value that `lastUpdated` uses the first time Mule executes the poll, or whenever the watermark can't be found.
.. `selector="MAX"` - Pick the MAX value returned by the Selector Expression to update the `lastUpdatedVal` variable each time the flow execution completes. In this case, it's the maximum of the lastUpdated value in the generated output (that is, the most recent one).
.. `selector-expression="#[payload.lastUpdated]"` - Returns the lastUpdated value of each object in the generated output, this value is passed on to the Selector.
. Add `where LastUpdated > #[flowVars.lastUpdatedVal]` to the Database connector statement:
+
[source, xml]
----
<db:select config-ref="MySQL_Configuration" doc:name="Database">
...
----
+
The `lastUpdatedVal` value is a string or Mule expression |Instructs the connector to return only those columns with an LastUpdated value greater than the value of the `lastUpdatedVal` variable. `lastUpdatedVal` is a flow variable that Mule creates, then updates every time the poll runs.
+
...
[source]
----
#[flowVars['lastUpdatedVal']] = #[payload.LastUpdated]
----
